...
export async function POST(req) {
    try {
        ...
        if (!driver) return NextResponse.json({ message: "Invalid credentials" }, { status: 401 });
        if (driver.status !== "approved") return NextResponse.json({ message: "Your account is not approved yet. Contact admin." }, { status: 403 });
        ...
        const response = NextResponse.json({ message: "Login successful", user: { id: driver._id, name: driver.name, phone: driver.phone, rickshawNumber: driver.rickshawNumber, isAvailable: driver.isAvailable, role: "driver" } });
