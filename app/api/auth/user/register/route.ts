...
export async function POST(request: NextRequest) {
    try {
        ...
        // Check if phone or email already exists (only verified users block registration)
        ...
        // Store OTP + pending user data together (user is created only after OTP verification)
        await Otp.deleteMany({ email, purpose: "register" }); // clear old OTPs
        await Otp.create({
            email,
            code,
            purpose: "register",
            expiresAt,
            pendingUser: { name, phone, email, password: hashedPassword },
        });
