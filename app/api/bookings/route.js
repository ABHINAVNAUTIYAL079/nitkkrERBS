...
export async function POST(req) {
    try {
        ...
        if (!studentName || !phone || !pickupLocation || !dropLocation) return NextResponse.json({ message: "All fields are required" }, { status: 400 });
        if (pickupLocation === dropLocation) return NextResponse.json({ message: "Pickup and drop locations must be different" }, { status: 400 });
        if (!/^\d{10}$/.test(phone)) return NextResponse.json({ message: "Please enter a valid 10-digit phone number" }, { status: 400 });
...
export async function GET(req) {
    try {
        ...
        if (user?.role === "admin") {
            ...
            const bookings = await Booking.find(query).sort({ createdAt: -1 }).skip((page - 1) * limit).limit(limit).lean();
