...
export async function GET() {
    try {
        ...
        if (!existingAdmin) {
            ...
            await Admin.create({ name: "NIT KKR Admin", email: "admin@nitkkr.ac.in", password: hashedPassword, role: "admin" });
            adminStatus = "created";
        }
        let locationsSeeded = 0;
        for (const loc of campusLocations) {
            const existing = await Location.findOne({ name: loc.name });
            if (!existing) { await Location.create(loc); locationsSeeded++; }
        }
        return NextResponse.json({ success: true, message: "Setup complete!", admin: { status: adminStatus, email: "admin@nitkkr.ac.in", password: "admin123", note: "Change password after first login!" }, locations: { seeded: locationsSeeded, total: campusLocations.length } });
    } catch (error) {
