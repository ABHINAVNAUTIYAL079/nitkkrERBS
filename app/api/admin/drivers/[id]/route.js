...
export async function PATCH(req, { params }) {
    try {
        ...
        if (!["approved", "suspended", "pending"].includes(status)) return NextResponse.json({ message: "Invalid status" }, { status: 400 });
        const driver = await Driver.findByIdAndUpdate(params.id, { status }, { new: true }).select("-password");
