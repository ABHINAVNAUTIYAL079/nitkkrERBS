...
export default function DriverDashboard() {
    ...
    const [myRide, setMyRide] = useState<Booking | null>(null);
    ...
    const fetchHistory = useCallback(async () => {
        // Fetch all bookings assigned to this driver (completed/cancelled)
        const res = await fetch("/api/driver/history");
        if (res.ok) {
            const data = await res.json();
            setRideHistory(data.bookings || []);
        }
    }, []);
    ...
    const handleStatusUpdate = async (status: string) => {
        ...
        try {
            const res = await fetch(`/api/bookings/${myRide._id}`, {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
            ...
            if (status === "completed") {
                ...
                toast.success("Ride completed! You are now available.");
                fetchPending();
            } else {
                ...
                <div className="max-w-5xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl flex items-center justify-center">
                            <img src="/logo.png" alt="NIT KKR E-Rickshaw Logo" className="w-10 h-10 rounded-xl object-cover" />
                        </div>
                        <div>
                            <p className="font-bold text-white">{driver?.name}</p>
                            <p className="text-xs text-slate-400">🚗 {driver?.rickshawNumber}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        <button
            ...
            <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
                    ...
                    <div className="flex items-center justify-between flex-wrap gap-4">
                        <div>
                            ...
                            <div className="flex items-center gap-2">
                                <div
                                    className={`w-2.5 h-2.5 rounded-full ${driver?.isAvailable ? "bg-green-400 animate-pulse" : "bg-slate-500"
                                        }`}
                                />
                        ...
                        <button
                            ...
                            className={`px-5 py-2.5 rounded-xl font-semibold text-sm transition-all ${driver?.isAvailable
                    ...
                    <div className="bg-gradient-to-br from-blue-900/50 to-blue-800/30 border border-blue-700/50 rounded-2xl p-5">
                        ...
                        <div className="space-y-2 mb-4">
                            ...
                            <div className="flex items-center gap-2 text-sm mt-2">
                                ...
                                <span className="text-green-300">{myRide.pickupLocation}</span>
                                <span className="text-slate-500">→</span>
                                <MapPin className="w-4 h-4 text-red-400" />
                                <span className="text-red-300">{myRide.dropLocation}</span>
                            </div>
                        </div>
                        <div className="flex gap-3 flex-wrap">
                            {myRide.status === "accepted" && (
                                <button
                                    ...
                                    disabled={!!updating}
                                    className="flex items-center gap-2 px-4 py-2 bg-purple-500 hover:bg-purple-600 text-white font-semibold rounded-xl text-sm transition-colors disabled:opacity-60"
                                >
                                    {updating ? <Spinner size="sm" /> : <Navigation className="w-4 h-4" />}
                                    On the Way
                                </button>
                            )}
                            {myRide.status === "on_the_way" && (
                                <button
                                    ...
                                    disabled={!!updating}
                                    className="flex items-center gap-2 px-4 py-2 bg-green-500 hover:bg-green-600 text-white font-semibold rounded-xl text-sm transition-colors disabled:opacity-60"
                                >
                                    {updating ? <Spinner size="sm" /> : <CheckCircle className="w-4 h-4" />}
                                    Mark Completed
                                </button>
                            )}
                        </div>
                ...
                <div className="flex gap-2 bg-slate-800/80 rounded-2xl p-1.5 border border-slate-700">
                    <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold transition-all ${activeTab === "rides"
                    ...
                    >
                        ...
                        Pending Bookings
                        {bookings.length > 0 && (
                            <span className="bg-yellow-500 text-yellow-950 text-xs px-1.5 py-0.5 rounded-full font-bold">{bookings.length}</span>
                        )}
                    </button>
                    <button
                    ...
                    >
                        ...
                        {rideHistory.length > 0 && (
                            <span className="bg-slate-600 text-slate-200 text-xs px-1.5 py-0.5 rounded-full">{rideHistory.length}</span>
                        )}
                    </button>
                </div>
                        ...
                        {bookings.length === 0 ? (
                            <div className="bg-slate-800 rounded-2xl p-10 text-center border border-slate-700">
                                ...
                                <p className="text-slate-400">
                                    {myRide ? "You have an active ride." : "No pending bookings right now."}
                                </p>
                                    ...
                                    >
                                        <div className="flex flex-wrap items-start justify-between gap-4">
                                            ...
                                            <button
                                                ...
                                                disabled={!!updating || !!myRide || !driver?.isAvailable}
                                                className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white font-semibold rounded-xl text-sm transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                                            >
                        ...
                        {rideHistory.length === 0 ? (
                            ...
                            <div className="space-y-3">
                                {rideHistory.map((ride, index) => (
                                    <div key={ride._id || ride.bookingId} className="bg-slate-700/50 border border-slate-600/50 rounded-xl p-4 hover:border-slate-500 transition-colors">
                                        <div className="flex items-start justify-between gap-3 flex-wrap">
                                            <div className="flex-1 min-w-0 space-y-1">
                                                <div className="flex items-center gap-2">
                                                    <span className="text-xs text-slate-500 font-mono bg-slate-600/50 px-1.5 py-0.5 rounded">
                                                        #{rideHistory.length - index}
                                                    </span>
                                                ...
                                                <div className="flex items-center gap-2 text-sm">
                                                    <MapPin className="w-3.5 h-3.5 text-green-400" />
                                                    <span className="text-green-300 text-sm">{ride.pickupLocation}</span>
                                                    <span className="text-slate-500">→</span>
                                                    <MapPin className="w-3.5 h-3.5 text-red-400" />
                                                    <span className="text-red-300 text-sm">{ride.dropLocation}</span>
                                                </div>
